...
Where <prefix> is where Your program will install to. Usually it's F</usr> on UNIX like systems and 
F<C:\Program Files> on Windows ones.
...
=head2 F<Config.t> and F<Config.t.d>

For more elaborate results, use a F<Config.t> or F<Config.t.d>. The 
Config.t is a script that outputs in TAP format (see 
L<Test::Harness::TAP> . Here is a dummy written in standard SH.
...
Adding to the Config.h is done by writing to Config.h.part. Any code added 
to it will be appended to Config.h. For example
...
You can write this in any language (I used shell since almost anyone knows 
it). 

=head2 Config.t.d

This directory contains F<.t> files. They contain the same syntax that 
F<Config.t> does, though it is easier to manage multiple files that test 
multiple features.
